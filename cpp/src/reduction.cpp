...
void ReduceLocalTask::cpu_variant(legate::TaskContext context)
{
...
class AggregationHelper final {
 public:
   ...
   */
  void add_first_pass(std::string op, std::shared_ptr<arrow::DataType> output_type)
  {
  ...
  LogicalColumn get_first_pass_result(std::string op, std::shared_ptr<arrow::DataType> output_type)
  {
    AggregationDescriptor agg_descr{op, output_type, std::nullopt};
   ...
   */
  void breakdown_aggregation(std::string op, std::shared_ptr<arrow::DataType> output_type)
  {
    if (op == "mean") {
      /* Mean calculates it's final result from the final sum and counts. */
      add("sum", output_type, std::nullopt);
      add("count_valid", arrow::int64(), std::nullopt);
    } else {
    ...
    std::optional<std::reference_wrapper<const LogicalColumn>> initial)
  {
    if (op == "mean") {
      ...
      auto counts = get_result("count_valid", arrow::int64(), std::nullopt);
