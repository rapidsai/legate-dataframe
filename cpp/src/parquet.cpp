...
namespace legate::dataframe
{
                            ...
                            const std::optional<std::vector<std::string>>& columns)
  {
    ...
    for (int i = 0; i < info.column_types.size(); i++) {
      logical_columns.emplace_back(LogicalColumn::empty_like(
        info.column_types.at(i), info.column_nullable.at(i), false, size));
    }
      ...
      if (!nullable) {
        if (null_value.type() != legate_type) {
          throw std::invalid_argument(
            "null value must be null or have the same type as the result");
        }
      }
    }
